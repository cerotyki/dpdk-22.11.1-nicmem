	...
	if (priv->mtr_profile_arr) {
		if (meter_profile_id >= priv->mtr_config.nb_meter_profiles)
			return -rte_mtr_error_set(error, EINVAL,
					...
					NULL, "Meter profile id not valid1.");
		...
		if (meter_profile_id == UINT32_MAX)
			return -rte_mtr_error_set(error, EINVAL,
					RTE_MTR_ERROR_TYPE_METER_PROFILE_ID,
					NULL, "Meter profile id not valid2.");
		fmp = mlx5_flow_meter_profile_find(priv, meter_profile_id);
		/* Meter profile must not exist. */
		if (fmp)
			return -rte_mtr_error_set(error, EEXIST,
		       ...
		       struct rte_mtr_error *error)
{
	...
	printf("meter_profile_add: 0\n");
	if (!priv->mtr_en)
		return -rte_mtr_error_set(error, ENOTSUP,
					  RTE_MTR_ERROR_TYPE_UNSPECIFIED, NULL,
					  "Meter is not supported");
	printf("meter_profile_add: 1\n");
	/* Check input params. */
	ret = mlx5_flow_meter_profile_validate(dev, meter_profile_id,
					       profile, error);
	if (ret) {
		printf("meter_profile_add: 1.5\n");
		return ret;
	}
	printf("meter_profile_add: 2\n");
	...
	printf("meter_profile_add: 5, meter_profile_id: %d\n", meter_profile_id);
	return 0;
error:
	mlx5_free(fmp);
	return ret;
			...
			struct rte_mtr_error *error)
{
	...
	if (policy_id == priv->sh->mtrmng->def_policy_id) {
		printf("def_policy_id: %d\n", priv->sh->mtrmng->def_policy_id);
		return -rte_mtr_error_set(error, EEXIST,
					  RTE_MTR_ERROR_TYPE_METER_POLICY_ID,
		       ...
		       struct rte_mtr_error *error)
{
	...
	if (fmp == NULL)
		return -rte_mtr_error_set(error, ENOENT,
			RTE_MTR_ERROR_TYPE_METER_PROFILE_ID,
			NULL, "Meter profile id not valid4.");
	/* Meter policy must exist. */
		       ...
		       struct rte_mtr_error *error)
{
	...
	if (!profile->initialized)
		return -rte_mtr_error_set(error, ENOENT,
			RTE_MTR_ERROR_TYPE_METER_PROFILE_ID,
			NULL, "Meter profile id not valid5.");
	/* Meter policy must exist. */

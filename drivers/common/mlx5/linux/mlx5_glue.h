...
// #define MLX5_GLUE_MEASURE_DELAY
#ifdef MLX5_GLUE_MEASURE_DELAY
#include <time.h>
// #define TIME_FLAG (CLOCK_MONOTONIC)
// #define TIME_FLAG (CLOCK_THREAD_CPUTIME_ID)
#define NANOS (1000000000)
#define MICROS (1000000)
#define MEASURE(name, cmd) do { \
	...
	_time1 = (_end1.tv_sec - _start1.tv_sec) * NANOS + (_end1.tv_nsec - _start1.tv_nsec); \
	_time2 = (_end2.tv_sec - _start2.tv_sec) * NANOS + (_end2.tv_nsec - _start2.tv_nsec); \
	printf("[%s] Wallclock time: %lf, Thread time: %lf\n", \
		name, (double)_time1 / NANOS * MICROS, (double)_time2 / NANOS * MICROS); \
} while (/*CONSTCOND*/0)
#else
#define MEASURE(name, cmd) do { \
	cmd \

...
const char *g_mac_addr[MAX_IP] = {
    "08:c0:eb:62:43:f0", "b8:ce:f6:d2:ca:4a", // core2 core3
    "0c:42:a1:ca:e8:6c", "10:70:fd:86:5c:8a", // box1 box3
    "98:03:9b:1e:dc:8c", "b8:ce:f6:d2:ca:46", // box2 box4
    "e8:eb:d3:a7:32:f3", "98:03:9b:7f:c4:90", // protein1 forest1
	NULL,
};
uint32_t g_ip_list[MAX_IP];
uint8_t g_mac_list[MAX_IP][RTE_ETHER_ADDR_LEN];
int g_num_neighbor = 0;
...
print_stats(uint64_t timer_tsc)
{
	...
	uint64_t total_packets_tx_bytes, total_packets_rx_bytes;
	unsigned portid;

	total_packets_dropped = 0;
	total_packets_tx = 0;
	total_packets_rx = 0;
	total_packets_tx_bytes = 0;
	...
	for (portid = 0; portid < RTE_MAX_ETHPORTS; portid++) {
		...
		printf("\nStatistics for port %u ------------------------------"
			   ...
			   "\nTX Throughput: %23"PRIu64
			   "\nRX Throughput: %23"PRIu64
			   "\nPackets dropped: %21"PRIu64,
			   portid,
			   port_statistics[portid].tx,
			   port_statistics[portid].rx,
			   port_statistics[portid].tx_bytes * 8 / 1000000000,
			   port_statistics[portid].rx_bytes * 8 / 1000000000,
			   port_statistics[portid].dropped);

		total_packets_dropped += port_statistics[portid].dropped;
		total_packets_tx += port_statistics[portid].tx;
		total_packets_rx += port_statistics[portid].rx;
		total_packets_tx_bytes += port_statistics[portid].tx_bytes;
		total_packets_rx_bytes += port_statistics[portid].rx_bytes;

		port_statistics[portid].tx = port_statistics[portid].rx = 
			port_statistics[portid].tx_bytes = port_statistics[portid].rx_bytes = 0;
	}
	printf("\nAggregate statistics ==============================="
		   "\nTotal packets sent: %18"PRIu64
		   "\nTotal packets received: %14"PRIu64
		   "\nTotal TX throughput: %17"PRIu64
		   "\nTotal RX throughput: %17"PRIu64
		   "\nTotal packets dropped: %15"PRIu64,
		   total_packets_tx,
		   total_packets_rx,
		   total_packets_tx_bytes * 8 / 1000000000,
...
static inline void 
convert_mac(const char *mac_string, uint8_t *mac_array) {
    int i, byte;
    char byteStr[3];

    for (i = 0; i < 6; i++) {
        strncpy(byteStr, mac_string + i * 3, 2);
        byteStr[2] = '\0';  // Null-terminate the string
        sscanf(byteStr, "%x", &byte);  // Convert the hex string to a byte
        mac_array[i] = (uint8_t)byte;
    }
}
...
l2fwd_mac_updating(struct rte_mbuf *m, unsigned dest_portid)
{
    ...
    for (i = 0; i < g_num_neighbor; i++)
    {
        if (iph->dst_addr == g_ip_list[i]) {
    		rte_ether_addr_copy((const struct rte_ether_addr *)g_mac_list[i], &eth->dst_addr);
...
l2fwd_simple_forward(struct rte_mbuf *m, unsigned portid)
{
	...
	if (sent) {
		port_statistics[dst_port].tx_bytes += m->pkt_len;
...
l2fwd_main_loop(void)
{
	...
	unsigned i, j, portid, nb_rx, temp;
	...
	while (!force_quit) {
		...
		if (unlikely(diff_tsc > drain_tsc)) {
			...
			for (i = 0; i < qconf->n_rx_port; i++) {
				...
				for (int j = 0; j < buffer->length; j++) {
					port_statistics[portid].tx_bytes += buffer->pkts[j]->pkt_len;
				}
		...
		for (i = 0; i < qconf->n_rx_port; i++) {
			...
			nb_rx = 0;
			// while (true) {
			// 	temp = rte_eth_rx_burst(portid, 0, pkts_burst + nb_rx, MAX_PKT_BURST);
			// 	nb_rx += temp;
			...
			for (j = 0; j < nb_rx; j++) {
				m = pkts_burst[j];
				port_statistics[portid].rx_bytes += m->pkt_len;
...
main(int argc, char **argv)
{
    ...
    for (int i = 0; i < MAX_IP; i++) {
		if (g_ip_addr[i]) {
			g_ip_list[i] = inet_addr(g_ip_addr[i]);
			convert_mac(g_mac_addr[i], g_mac_list[i]);
			g_num_neighbor++;
		}
	}
	...
	l2fwd_pktmbuf_pool = rte_pktmbuf_pool_create("mbuf_pool", nb_mbufs,
		MEMPOOL_CACHE_SIZE, 0, MAX_MBUF_SIZE,
